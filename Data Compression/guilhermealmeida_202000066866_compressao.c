...
MinHeap* criar_min_heap(unsigned capacidade) {
    ...
    min_heap->array = malloc(min_heap->capacidade * sizeof(MinHeapNode*));
    return min_heap;
}
...
void min_heapify(MinHeap* min_heap, int idx) {
    ...
    if ((unsigned)esq < min_heap->tamanho && 
        eh_prioritario(min_heap->array[esq], min_heap->array[menor]))
        menor = esq;

    if ((unsigned)dir < min_heap->tamanho && 
        eh_prioritario(min_heap->array[dir], min_heap->array[menor]))
        menor = dir;
...
MinHeapNode* construir_arvore_huffman(unsigned char dados[], int freq[], int tamanho) {
    ...
    for (int i = 0; i < tamanho; ++i) {
        min_heap->array[i] = novo_no(dados[i], freq[i]);
    }
    min_heap->tamanho = tamanho;

    for (int i = (min_heap->tamanho - 2) / 2; i >= 0; --i) {
...
void gerar_codigos(MinHeapNode* raiz, int arr[], int topo, char* codigos[256]) {
    ...
    if (raiz->is_folha) {
        codigos[raiz->dado] = malloc((topo + 1) * sizeof(char));
...
ResultadoCompressao* executar_huffman(ByteBuffer* entrada) {
    ...
    if (entrada->tamanho == 0) return res;
