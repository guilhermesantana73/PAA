#include <stdio.h>
#include <stdlib.h> // Agora PERMITIDO (para malloc, free, exit, abs)
#include <string.h> 
...
Registro* buscaBinaria(const char* chave, Registro listaOrdenada[], int tamanho);
...
Registro* buscaBinaria(const char* chave, Registro listaOrdenada[], int tamanho) {
    ...
    while (esquerda <= direita) {
        ...
        int cmp = strcmp(chave, listaOrdenada[meio].codigo);
...
void processarListas(FILE* saida_cnpj, FILE* saida_peso, 
    ...
    for (int i = 0; i < tam_mestre; i++) {
        ...
        Registro* item_inspecionado = buscaBinaria(
            item_mestre->codigo,
